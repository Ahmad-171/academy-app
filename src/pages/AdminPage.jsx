    ...
    {productModal && (
      ...
      {modal === "form" && (
        <Modal title={editId ? "✏️ تعديل الحساب" : "➕ إضافة حساب جديد"} onClose={() => setModal(null)} wide={isDesktop}>
          ...
          {/* حساب مخفي — يظهر لحساب المبرمج فقط */}
          {canSeeHidden && (
            <div onClick={() => setForm(p => ({ ...p, hidden: !p.hidden }))}
              ...
              <div>
                <div style={{ fontSize: 13, fontWeight: 700, color: form.hidden ? COLORS.purple : COLORS.textPrimary }}>🙈 حساب مخفي</div>
                <div style={{ fontSize: 11, color: COLORS.textSecondary }}>لا يظهر إلا لحساب المبرمج (يدخل ويعمل بشكل طبيعي)</div>
              </div>
            </div>
          )}
