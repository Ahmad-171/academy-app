-- ════════════════════════════════════════════════════════════════════
-- NZ Academy — إعداد نظيف كامل لقاعدة البيانات (ملف واحد)
-- ⚠️ يحذف كل بيانات التطبيق ويعيد بناء المخطط الصحيح + الحماية + حساب المدير.
--     لا يمكن التراجع. شغّله مرة واحدة في Supabase → SQL Editor.
-- بعد تشغيله: تسجيل الدخول برقم الهوية 111 وكلمة السر 111.
-- ════════════════════════════════════════════════════════════════════

create extension if not exists pgcrypto;

-- ── 0) حذف الجداول القديمة (بما فيها الزائدة) وسياساتها ──
...
create table public.users (
  ...
  role               text not null default 'لاعب',
  custom_role        text,
  name               text not null,
  phone              text,
...
create or replace function my_id() returns text as $$
  select id from public.users where auth_uid = auth.uid() limit 1;
$$ language sql security definer stable;

create or replace function my_child_id() returns text as $$
...
-- ── 5) بيانات ابتدائية بسيطة ──
insert into public.discount_codes (code, percent_off, active) values ('WELCOME10', 10, true)
on conflict (code) do nothing;
insert into public.products (name, price, category, img, images)
values ('طقم الأكاديمية', 280, 'ملابس', '👕', '[]'::jsonb);
insert into public.settings (key, value) values ('director_message', 'نؤمن بأن كل موهبة تستحق الرعاية والتطوير. مرحبًا بكم في أكاديميتنا.')
on conflict (key) do nothing;

-- ── 6) إنشاء حساب المدير 111 / 111 ──
do $$
declare
  v_id text := '111'; v_pass text := '111'; v_email text; v_uid uuid;
begin
  v_email := v_id || '@academy.local';
  ...
  insert into public.users (id, auth_uid, name, role, custom_role, status, permissions)
  values (v_id, v_uid, 'المدير', 'مدير', 'مدير', 'نشط', '{}'::jsonb)
  on conflict (id) do update set auth_uid = excluded.auth_uid, role = 'مدير', status = 'نشط';
end $$;
